...
export default function CocktailDetail() {
    ...
    async function getDetails() {
      fetch(`https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${id}`)
        ...
        .then((data) => {
          ...
          checkIfFavorited(data.drinks[0].idDrink)
          setLoading(false)
        })
        .catch((error) => {
          setLoading(false)
  ...
  function checkIfFavorited(id) {
    ...
    if (favorites.some(favorite => Number(favorite.idDrink) === id)){
      setIsFavorite(true)
    }
  }
    ...
    <div className="text-black bg-white bg-opacity-90 flex-grow items-center min-h-[100vh] h-fit rounded-sm border-t-4 border-pink-900 mb-5">
      {!loading && details ? (
        <div className="flex flex-col m-auto p-10">
          ...
          <div className="flex flex-col md:flex-row items-center mt-10 border md:space-x-10 space-x-5">
